...
void Arm7Bot::moveIK3(PVector j5) {
  ...
  ARMPORT.print("IK3_status= ");
  ARMPORT.println(IK_status);
  if (IK_status == 0) {
    for (int i = 0; i < 3; i++) {
...
void Arm7Bot::moveIK5(PVector j6, PVector vec56) {
  ...
  ARMPORT.print("IK5_status= ");
  ARMPORT.println(IK_status);
  if (IK_status == 0) {
	ARMPORT.print("posG={");
    for (int i = 0; i < 5; i++) {
      posG[i] = degrees(theta[i]);
	  ARMPORT.print(posG[i]);
	  ARMPORT.print(",");
    }
	ARMPORT.println("}");
    posG[5] = theta5;
...
int Arm7Bot::IK3(PVector pt) {
  ...
  theta[2] += 1.134464;
  ARMPORT.print("theta[1]=");
  ARMPORT.print(theta[1]);
  ARMPORT.print(" theta[2]=");
  ARMPORT.println(theta[2]);
